...
//
// History:
//   - 2026-05-13 a: opened up worker-src blob: + tile CDNs so MapLibre could
//     load. That incident note lives in git if we ever reinstate the map.
//   - 2026-05-13 b: map removed (GFW users saw blank tiles). CSP tightened
//     back down — no more tile-CDN connect-src, no more blob: worker-src.
const csp = [
  "default-src 'self'",
  ...
  "font-src 'self' data:",
  // No workers in use any more.
  "worker-src 'self'",
  // First-party only. We deliberately do NOT allow `https:` wildcard —
  // keeps us safe from accidental data exfiltration.
